...
7) The user must provide the angle of the "main pupil axis" in radians, positive
...
8) The user must provide the number of edges of the polygon pupil. This
...
9) The user must provide the spider angle in radians. The spider angle affects
   the clocking of all the spider arms relative to the "main pupil axis", i.e.
   to the rest of the pupil.

10) The user must provide the width of each spider arms, expressed as a fraction
...
11) The user must provide the offsets of the line of each spider arm wrt the
...
15) The user must provide the wavelength of the light in [m].

16) The user must provide the focal ratio of the setup forming the images of the
...
17) The user must provide the size of the pixel in [m].

18) The user must provide the percentage of edge blur in [%] applied to the
...
19) The user must provide the FWHM of the object in [pixels]. The object is
...
mysetup = div.Opticsetup(img_collection, xc=None, yc=None, N=None, # image data can be used "as is"
                        ...
                        spiderArms_m=[0.035, 0.035],    # 2 arms of 3.5% of pupil diam ..
                        spiderOffset_m=[0.0, 0.0],      # .. pointing toward the centre
...
displayed with the plt.imshow(.T, origin='lower') directive, using imperatively
a transpose and the origin='lower' option.
...
def pli(im, *args, **kwargs):
    ...
    a coefficient [i,j] of an image appears at (i,j) coordinates on the graphic
    window, and superimposes naturally and logically with the axes of any
